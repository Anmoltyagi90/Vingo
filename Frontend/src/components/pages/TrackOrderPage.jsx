...
import DeliveryBoyTracking from "../dashboard/DeliveryBoyTracking.jsx";
import DeliveryBoy from "../dashboard/DeliveryBoy.jsx";
...
const TrackOrderPage = () => {
  ...
  const handleGetOrder = async () => {
    try {
      const res = await axios.get(
        `${SERVER_ORDER}/get-order-by-id/${orderId}`,
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 flex flex-col gap-6">
      <div
        className="absolute flex items-center gap-4 top-[20px] left-[20px] z-[10] mb-[10px]"
        onClick={() => navigate("/")}
      >
      ...
      {currentOrder?.shopOrders?.map((shopOrder, index) => (
        <div
          key={index}
          className="bg-white p-4 rounded-2xl shadow-2xl border border-orange-200 space-y-4 mt-18"
        >
          <div>
